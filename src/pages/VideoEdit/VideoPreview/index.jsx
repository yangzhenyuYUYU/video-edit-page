...
const TextElement = ({ 
  ...
  // 当元素被选中时，显示编辑界面和触发轨道项目选择
  useEffect(() => {
    if (isSelected) {
      // 触发自定义事件，通知轨道编辑器该元素被选中了
  ...
  const textStyle = {
    ...
    fontSize: item.textStyle?.fontSize ? `${item.textStyle.fontSize}px` : '24px',
    ...
    >
      ...
      {isSelected && (
        <div className="resize-rotate-handles">
          ...
          {/* 旋转控制点 */}
          <div className="rotate-handle" onMouseDown={(e) => { e.stopPropagation(); onRotateStart(e); }} />
        </div>
      )}
    </div>
  );
...
const ImageElement = ({
  ...
  return (
    ...
    >
      ...
      {isSelected && (
        <div className="resize-rotate-handles" style={{ pointerEvents: 'none' }}>
          {/* 缩放控制点 */}
          <div className="handle nw" style={{ pointerEvents: 'auto' }} onMouseDown={(e) => { e.stopPropagation(); onResizeStart(e, 'nw'); }} />
          <div className="handle ne" style={{ pointerEvents: 'auto' }} onMouseDown={(e) => { e.stopPropagation(); onResizeStart(e, 'ne'); }} />
          <div className="handle se" style={{ pointerEvents: 'auto' }} onMouseDown={(e) => { e.stopPropagation(); onResizeStart(e, 'se'); }} />
          <div className="handle sw" style={{ pointerEvents: 'auto' }} onMouseDown={(e) => { e.stopPropagation(); onResizeStart(e, 'sw'); }} />
          
          {/* 旋转控制点 */}
          <div className="rotate-handle" style={{ pointerEvents: 'auto' }} onMouseDown={(e) => { e.stopPropagation(); onRotateStart(e); }} />
        </div>
      )}
    </div>
  );
...
const VideoPreview = ({ 
  ...
  const handleRotateStart = (e) => {
    ...
    // 获取所选元素的中心点 - 现在元素中心就是定位坐标点，因为我们使用translate(-50%, -50%)
    let centerX, centerY;
    
    // 直接使用计算的坐标点作为中心点
    const containerRect = containerRef.current.getBoundingClientRect();
    centerX = containerRect.left + (selectedItem.x / 100) * containerSize.width;
    centerY = containerRect.top + (selectedItem.y / 100) * containerSize.height;
    ...
    const onMouseMove = (moveEvent) => {
      ...
      const currentAngle = Math.atan2(
        moveEvent.clientY - rotateStart.centerY,
        moveEvent.clientX - rotateStart.centerX
      ) * (180 / Math.PI);
      
      // 计算角度差
      let angleDifference = currentAngle - rotateStart.initialAngle;
      
      // 确定最终旋转角度（保持在0-360范围内）
      let finalRotation = (rotateStart.startRotation + angleDifference) % 360;
      if (finalRotation < 0) finalRotation += 360;
      
      console.log('Rotation:', { angleDifference, finalRotation });
